...
def run(
    ...
    modalities = {
        "text":  (os.path.join(data_dir, "text"),   TEXT_EXTS),
        "image": (os.path.join(data_dir, "images"), IMAGE_EXTS),
        "audio": (os.path.join(data_dir, "audio"),  AUDIO_EXTS),
    }
    ...
    for modality, (folder, exts) in modalities.items():
        ...
        out_path = os.path.join(output_dir, f"{modality}_metadata.csv")
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        help="Root data directory containing text/, images/, audio/ (default: data/)"
    )
    parser.add_argument(
        "--output_dir", default="outputs/indexes",
        help="Where to save the metadata CSVs (default: outputs/indexes/)"
    )
    parser.add_argument(
